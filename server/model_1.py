...
#		self.softmax = nn.Softmax(dim=1)
	...
	def forward(self, x):
		#  Level 1 context pathway
		out = self.conv3d_c1_1(x) # y: 1-16-180-220-120
		residual_1 = out
		out = self.lrelu(out) 
		...
		out += residual_1
		context_1 = self.lrelu(out)# context_1: 1-16-180-220-120
		out = self.inorm3d_c1(out)
		out = self.lrelu(out)

		# Level 2 context pathway
		out = self.conv3d_c2(out) # y: 1-32-90-110-60
		residual_2 = out
		...
		out = self.lrelu(out)
		context_2 = out # context_2: 1-32-90-110-60

		# Level 3 context pathway
		out = self.conv3d_c3(out) # y: 1-64-45-55-30
		residual_3 = out
		...
		out = self.lrelu(out)
		context_3 = out # context_3: 1-64-45-55-30

		# Level 4 context pathway
		out = self.conv3d_c4(out) # y: 1-128-23-28-15
		residual_4 = out
...
#		pdb.set_trace()
		# Level 5
		out = self.conv3d_c5(out) # y: 1-256-12-14-8
		residual_5 = out
...
#		print("context_4:", context_4.size())
		out = F.interpolate(out, context_4.size()[2:], mode='nearest')# y: 1-128-23-28-15
...
#		print("out_6: ", out.size(), '\ncontext_4:', context_4.size())
		out = torch.cat([out, context_4], dim=1) #y 1-256-23-28-15
		out = self.conv_norm_lrelu_l1(out)
		out = self.conv3d_l1(out) #y : 1-128-23-28-15
		out = self.norm_lrelu_upscale_conv_norm_lrelu_l1(out) #y 1-64-46-56-30

		# Level 2 localization pathway
#		print("out_7: ", out.size(), '\ncontext_3:', context_3.size()) # torch.Size([2, 64, 46, 56, 30]) 
		out = F.interpolate(out, context_3.size()[2:], mode='nearest') #context_3: torch.Size([2, 64, 45, 55, 30])
